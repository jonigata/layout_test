...
public:
	...
	self_type&  operator=(const vector_type& av){v=av;return *this;}
        ...
        {return self_type(v+vector_type(o));}
        self_type   operator-(const self_type& o)const
        {return self_type(v-vector_type(o));}
...
public:
        ...
        self_type   operator*(const self_type& o)const
        {
                return self_type(v.x*o.width(),v.y*o.height());
        }
        self_type   operator/(const self_type& o)const
        {
                return self_type(v.x/o.width(),v.y/o.height());
...
public:
	...
	bool	contains(const offset_type& p)const
	{
		...
		z.normalize();
		return 
			z.left()<=p.x() && p.x()<z.right() && 
			z.top()<=p.y() && p.y()<z.bottom();
	}
	...
	offset_type	clip(const offset_type& o)const
	{
        ...
        self_type   inner_largest/*_similar_bounds*/(self_type ib)const
        {
                ib.normalize();
                self_type ob=*this;ob.normalize();
                self_type b(ob.extent().inner_largest(ib.extent()));
                b.center(ob.center());
                return b;
        }
        
        self_type   outer_smallest/*_similar_bounds*/(self_type ob)const
        {
                ob.nobmalize();
                self_type ib=*this;ib.nobmalize();
                self_type b(ib.extent().outer_smallest(ob.extent()));
                b.center(ib.center());
                return b;
        }
