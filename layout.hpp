/*!
  ...
  <説明>
  $Id: layout.hpp 20 2005-09-24 11:13:54Z naoyuki $
*/
#ifndef LAYOUT_HPP
#define LAYOUT_HPP
        ...
        public:
                ...
                int calc_physical_width(const extent_type& extent)const
                {
						...
						box_height=extent.h();
                                ...
                                return calc_fit_width(box_height); // 仮想関数呼び出し
                ...
                int calc_physical_height(const extent_type& extent)const
                {
						...
						box_width=extent.w();
                                ...
                                return calc_fit_height(box_width);
                ...
                virtual int calc_fit_width(int height)const=0;
                virtual int calc_fit_height(int width)const=0;
                virtual void set_physical_bounds(const bounds_type&)=0;
                virtual bool accept(detail::abstract_visitor_holder*)=0;
    
        protected:
                panel_imp(){extent_=extent_type(fit(),fit());}
...
protected:
        ...
        public:
                ...
                int calc_fit_width(int)const{return t_.width();}
                int calc_fit_height(int)const{return t_.height();}
...
public:
        ...
        public:
                ...
                int calc_fit_width(int h)const
                {
                ...
                int calc_fit_height(int w)const
                {
                ...
                bool accept(detail::abstract_visitor_holder* vh)
                {
                        bool f = false;
                        for( elements_type::const_iterator i = elements_.begin();
                             i != elements_.end();
                ...
                void add(imp_ptr x){elements_.push_back(x);}
...
public:
        ...
        {
                enunique();
                boost::polymorphic_downcast<layer_imp*>(
        ...
        public:
			...
			int ph	=calc_physical_height(b.extent());
                        ...
                        for( elements_type::const_iterator i = elements_.begin();
                             i != elements_.end();
                             ++i){
                                int ih=(*i)->get_logical_height();
                                if(infinity()==ih){
                                        inf++;
                                } else if(infinityx()==ih){
                                        infx++;
                                } else {
                                        ih=(*i)->calc_physical_height(b.extent());
                                        rh-=ih;
                                }
                        }
...
public:
        ...
        public:
                ...
                int calc_fit_width(int h)const
                {
                        ...
                        // 要素情報の取得
                        int box_width=get_logical_width(); // 非infinite要素の高さの合計
                        for( elements_type::const_iterator i = elements_.begin();
                             i != elements_.end();
                             ++i){
                                // 子に無限要素がある場合、↓の
                ...
                int calc_fit_height(int w)const
                {
                        ...
                        // 要素情報の取得
                        int box_height=get_logical_height(); // 非infinite要素の高さの合計
                        for( elements_type::const_iterator i = elements_.begin();
                             i != elements_.end();
                             ++i){
                                // 子に無限要素がある場合、↓の
			...
			int pw	=calc_physical_width(b.extent());
                        ...
                        for( elements_type::const_iterator i = elements_.begin();
                             i != elements_.end();
                             ++i){
                                int iw=(*i)->get_logical_width();
                                if(infinity()==iw){
                                        inf++;
                                } else if(infinityx()==iw){
                                        infx++;
                                } else {
                                        iw=(*i)->calc_physical_width(b.extent());
                                        rw-=iw;
                                }
                        }
...
public:
        ...
        {
                enunique();
                boost::polymorphic_downcast<vbox_base_imp*>(
                        &(*get_entity()))->add(friend_entity(x));
                return *this;
        }
...
public:
        ...
        {
                enunique();
                boost::polymorphic_downcast<hbox_base_imp*>(
                        &(*get_entity()))->add(friend_entity(x));
                return *this;
        }
...
protected:
        ...
        public:
                ...
                int calc_fit_width(int)const{return 0;}
                int calc_fit_height(int)const{return 0;}

                void set_physical_bounds(const bounds_type&){}

                bool accept(detail::abstract_visitor_holder*){return false;}
...
public:
        ...
        public:
                typedef table_row::table_row_imp::elements_type row_elements_type;
        ...
        public:
                ...
                void set_physical_bounds(const bounds_type& b)
                {
                        ...
                        for(size_t x=0;x<cols;x++){
                                logical_col_widths.push_back(get_column_logical_width(x));
                        }
                        ...
                        for(size_t y=0;y<rows;y++){
                                fith+=elements_[y]->calc_fit_height(fit());
                        }
        ...
        protected:
                ...
                int get_column_fit_width(int index,const extent_type& e)const
                {
                        ...
                        for(size_t y=0;y<elements_.size();y++){
                                ...
                                m=zfx::max(m,elements[index]->calc_fit_width(fit()));
                        }
...
public:
        ...
        {
                enunique();
                boost::polymorphic_downcast<table_imp*>(
                        &(*get_entity()))->add(friend_entity(x));
                return *this;
        }
